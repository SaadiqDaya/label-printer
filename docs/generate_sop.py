...
add_table(['Type', 'Produces'], [
    ...
    ['Database Field', "Mirrors a COLUMN of the connected data file under this source's name"],
], col_widths=[1.8, 4.2])
add_heading('11.1  Database Field Sources (survive data-file changes)', 2)
add_para('A Database Field source gives a data-file column a stable name. Set its **Source Column**, then bind elements to the SOURCE\'s name instead of the raw column: if the file\'s column changes, fix the mapping ONCE and every element follows; a value used in several places updates everywhere together. Elements can still bind directly to a raw column — the Bound Field dropdown lists both.')
add_heading('11.2  Formula Fields', 2)
...
add_note('Where do templates come from? The list shows every .lbl file in the station\'s Templates folder (📂 button opens it). Design and Save them there in the Designer, or point all stations at one shared folder (Section 19.2); press ⟳ to pick up new files.')
add_heading('13.1  Daily Loop', 2)
add_numbered('Type your name in the **Operator** box — it is remembered, and recorded on every print in the history.')
...
add_numbered('Each record has a **tick box** and an editable **quantity** ("(was N)" shows the file\'s original value; PrintQty-0 rows arrive unticked).')
add_numbered('**Pick a record** and **Print selected**, or set a **range** and **Print all in range** (every ticked row, at its quantity).')
add_numbered('**Skip invalid rows (report them)** — print the good rows instead of blocking the whole batch; every skipped row and its reason is listed.')
...
add_heading('13.2  The Job Queue (watch folders)', 2)
add_para('If IT has configured **watch folders** (Section 19.4), batch jobs sent by the ERP appear in the **Jobs** tab automatically.')
add_numbered('**Click a job** — its rows appear grouped by template (each group shows its printer). Tick rows and adjust quantities like a data file; click a row to preview it.')
add_numbered('SHEET jobs (templates with a Page Setup — Avery sheets, menus) preview the COMPOSED SHEETS with ◄ ► page buttons, updating live as you tick rows / change quantities; clicking a row jumps to its sheet.')
add_numbered('Rows that match no template are listed in red — they will not print; the rest of the job still can.')
add_numbered('**PRINT JOB** prints every ticked row. **Reject** moves the file to the failed folder without printing. **Close** leaves it in the queue.')
add_numbered('Finished jobs move to **printed\\** with a **.result.txt** report; failures move to **failed\\** with a **.error.txt**. Job files are never deleted.')
add_note('Folders set to auto-print skip the queue: valid jobs print on arrival; the status bar and history show the result.')
page_break()
...
add_heading('19.4  Watch Folders (ERP job drop — works with ANY ERP)', 2)
add_para('A watch folder lets any system print labels by **writing a CSV into a folder** — no integration code. Configure per station in **File > Settings > Watch folders** (applies when the Print Station next starts). Under the chosen root the app creates **inbox\\** (the ERP drops job CSVs here), **processing\\<STATION>\\** (a station claims a job by an atomic move — no double-printing, even with several stations on one folder), **printed\\** (completed jobs + .result.txt audit report) and **failed\\** (failed/rejected jobs + .error.txt). Files are moved, never deleted; crash leftovers return to the inbox on next start.')
